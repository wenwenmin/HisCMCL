...
def find_matches(spot_embeddings, query_embeddings, top_k=1):
    spot_embeddings = torch.tensor(spot_embeddings) if not isinstance(spot_embeddings, torch.Tensor) else spot_embeddings
    query_embeddings = torch.tensor(query_embeddings) if not isinstance(query_embeddings, torch.Tensor) else query_embeddings
...
if SAVE_EMBEDDINGS:
    for fold in range(32):
        save_embeddings(model_path=f"./model_result/her2st/{names[fold]}_uni/best_{fold}.pt",
...
for fold in range(32):
    ...
    save_path = f"./embedding_result/her2st_result/embeddings_{fold}/"
    spot_embeddings = [np.load(os.path.join(save_path, f"spot_embeddings_{i + 1}.npy")) for i in range(32)]
    image_embeddings = np.load(os.path.join(save_path, f"img_embeddings_{fold + 1}.npy"))
    ...
    # Print PCC for each slice
    print(f"Slice {fold + 1} - HGV PCC: {np.mean(hvg_pcc)}")
    print(f"Slice {fold + 1} - HEG PCC: {np.mean(heg_pcc)}")
...
print(f"avg heg pcc: {np.mean(heg_pcc_list)}")
print(f"avg hvg pcc: {np.mean(hvg_pcc_list)}")
print(f"avg mse: {np.mean(mse_list)}")
print(f"avg mae: {np.mean(mae_list)}")
