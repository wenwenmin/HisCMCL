...
def save_embeddings(model_path, save_path, datasize):
    ...
    model = mclSTExp_Attention(encoder_name=args.encoder_name,
                               ...
                               scales=[int(s) for s in args.scales.split(',')] if args.use_multiscale else [224],
...
if SAVE_EMBEDDINGS:
    # 从命令行获取参数并设置
    args = generate_args()
    args.use_multiscale = True  # 如果您使用了多尺度模型，设置为True
    args.scales = "224,672"  # 设置为与训练时相同的尺度
    ...
    for fold in range(12):
        ...
        model_path = f"./model_result/cscc/{names[fold]}{config_str}/best_{fold}.pt"
