...
class DATA_BRAIN(torch.utils.data.Dataset):
    def __init__(self, train=True, gene_list=None, ds=None, fold=0):
        ...
        gene_list = list(np.load('D:\dataset\DLPFC/brain_cut_1000.npy', allow_pickle=True))
        self.gene_list = gene_list
        self.train = train
...
class DATA_SKIN(torch.utils.data.Dataset):
    def __init__(self, train=True, gene_list=None, ds=None, fold=0):
        super(DATA_SKIN, self).__init__()
        self.dir = 'D:/dataset/CSCC'
        self.r = 224 // 2

        sample_names = [f"NCBI{i}" for i in range(759, 771)]  # NCBI759 to NCBI770

        gene_list = list(np.load('D:/dataset/CSCC/skin_hvg_cut_1000.npy', allow_pickle=True))
        self.gene_list = gene_list
        self.train = train
        samples = sample_names
        te_names = [samples[fold]]
    ...
    def get_img(self, name):
        image_path = os.path.join(self.dir, f"{name}/spatial/{name}.tif")
        im = Image.open(image_path)
        return im
    ...
    def get_adata(self, section_id):
        try:
            ...
            adata = sc.read_h5ad(os.path.join(self.dir, section_id, f"{section_id}.h5ad"))
    ...
    def get_pos(self, adata):
        ...
        image_spatial_df = pd.DataFrame(adata.obsm['spatial'], 
                                      index=adata.obs.index,
    ...
    def get_meta(self, section_id, gene_list=None):
        adata = self.get_adata(section_id)
        if adata is None:
            raise ValueError(f"Failed to load data for {section_id}")
            

        if 'spatial' not in adata.obsm:
            raise ValueError(f"No spatial coordinates found in {section_id}")
